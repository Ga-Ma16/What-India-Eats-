...
export const RAW_INDIAN_FOOD_CSV = `name,ingredients,diet,prep_time,cook_time,flavor_profile,course,state,region
...
export function parseCSV(csvText: string): RawDish[] {
  const lines = csvText.trim().split('\n');
  const results: RawDish[] = [];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (row.length >= 9) {
      const prep = parseInt(row[3], 10);
      const cook = parseInt(row[4], 10);
      results.push({
        name: row[0],
        ingredients: row[1],
        diet: row[2].trim(),
        prep_time: isNaN(prep) || prep === -1 ? null : prep,
        cook_time: isNaN(cook) || cook === -1 ? null : cook,
        flavor_profile: row[5].trim(),
        course: row[6].trim(),
        state: row[7].trim(),
        region: row[8].trim()
      });
